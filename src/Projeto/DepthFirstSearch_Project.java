...
public class DepthFirstSearch_Project {
     ...
     */
    public DepthFirstSearch_Project(SymbolGraphWheighted G, int s) {
        marked = new boolean[G.graph().V()];
        validateVertex(s);
        dfs(G, s);
    }

    // depth first search from v
    private void dfs(SymbolGraphWheighted G, int v) {
        count++;
        marked[v] = true;
        for (Edge w : G.graph().adj(v)) {
            if (!marked[w.other(v)]) {
                dfs(G, w.other(v));
            }
        }
